...
object ParamRanges {
  ...
  val map: Map[String, Info] = Map(
    // ---- Chaos ----
    "CuspN" -> Info(params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "CuspL" -> Info(params = Map(
      "xi" -> Spec(scalar = true)
    )),
    // FBSineN, FBSineL, FBSineC,
    "GbmanN" -> Info(params = Map(
      "xi" -> Spec(scalar = true),
      "yi" -> Spec(scalar = true)
    )),
    "GbmanL" -> Info(params = Map(
      "xi" -> Spec(scalar = true),
      "yi" -> Spec(scalar = true)
    )),
    "HenonN" -> Info(params = Map(
      "x0" -> Spec(scalar = true),
      "x1" -> Spec(scalar = true)
    )),
    "HenonL" -> Info(params = Map(
      "x0" -> Spec(scalar = true),
      "x1" -> Spec(scalar = true)
    )),
    "HenonC" -> Info(params = Map(
      "x0" -> Spec(scalar = true),
      "x1" -> Spec(scalar = true)
    )),
    "LatoocarfianN" -> Info(params = Map(
    ...
    "LinCongN" -> Info(outLo = -1.0, outHi = 1.0, params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "LinCongL" -> Info(outLo = -1.0, outHi = 1.0, params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "LinCongC" -> Info(outLo = -1.0, outHi = 1.0, params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "LorenzL" -> Info(params = Map(
      "h" -> Spec(lo = 0.0, hi = 0.06), // XXX TODO -- max h ?
      "xi" -> Spec(scalar = true),
      "yi" -> Spec(scalar = true),
      "zi" -> Spec(scalar = true)
    )),
    "QuadN" -> Info(params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "QuadL" -> Info(params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "QuadC" -> Info(params = Map(
      "xi" -> Spec(scalar = true)
    )),
    "StandardN" -> Info(params = Map(  // XXX TODO -- %2pi is a lie, with xi = 24 you get an initial value beyond that
      "xi" -> Spec(scalar = true),
      "yi" -> Spec(scalar = true)
    )),
    "StandardL" -> Info(params = Map(  // XXX TODO -- %2pi is a lie, with xi = 24 you get an initial value beyond that
      "xi" -> Spec(scalar = true),
      "yi" -> Spec(scalar = true)
    )),
    // ---- Delay ----
    ...
    // ---- DiskIO ----
    // ---- DynNoise ----
    "LFDNoise0" -> Info(outLo = -1.0, outHi = +1.0),
    "LFDNoise1" -> Info(outLo = -1.0, outHi = +1.0),
    "LFDNoise3" -> Info(outLo = -1.0, outHi = +1.0),
    "LFDClipNoise" -> Info(outLo = -1.0, outHi = +1.0),
    // ---- FFT2 ----
    ...
    "Lag2" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "time" -> Spec(lo = 0.0, hi = 30.0) // arbitrary hi
    )),
    "Lag3" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "time" -> Spec(lo = 0.0, hi = 30.0) // arbitrary hi
    )),
    "LagUD" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "timeUp" -> Spec(lo = 0.0, hi = 30.0), // arbitrary hi
      "timeDown" -> Spec(lo = 0.0, hi = 30.0) // arbitrary hi
    )),
    "Lag2UD" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "timeUp" -> Spec(lo = 0.0, hi = 30.0), // arbitrary hi
      "timeDown" -> Spec(lo = 0.0, hi = 30.0) // arbitrary hi
    )),
    "Lag3UD" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "timeUp" -> Spec(lo = 0.0, hi = 30.0), // arbitrary hi
      "timeDown" -> Spec(lo = 0.0, hi = 30.0) // arbitrary hi
    )),
    "OnePole" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "coeff" -> Spec(lo = -0.999, hi = +0.999)
    )),
    "OneZero" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "coeff" -> Spec(lo = -1.0, hi = +1.0)
    )),
    "TwoPole" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "freq" -> Spec(lo = 10.0, hi = 20000.0), // arbitrary
      "radius" -> Spec(lo = 0.0, hi = 1.0)
    )),
    "TwoZero" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "freq" -> Spec(lo = 10.0, hi = 20000.0), // arbitrary
      "radius" -> Spec(lo = 0.0, hi = 1.0)
    )),
    "Decay" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "time" -> Spec(lo = 0.0, hi = 30.0) // hi arbitrary
    )),
    "Decay2" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "attack" -> Spec(lo = 0.0, hi = 30.0), // hi arbitrary
      "release" -> Spec(lo = 0.0, hi = 30.0)  // hi arbitrary
    )),
    "Delay1" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "Delay2" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "Integrator" -> Info(params = Map(
      "in" -> Spec(dynamic = true),
      "coeff" -> Spec(lo = -0.999, hi = +0.999)
    )),
    "LeakDC" -> Info(params = Map(
      "coeff" -> Spec(lo = 0.8, hi = +0.99) // arbitrary
    )),
    "LPZ1" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "HPZ1" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "LPZ2" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "HPZ2" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "BPZ2" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    "BRZ2" -> Info(params = Map(
      "in" -> Spec(dynamic = true)
    )),
    // APF
    "LPF" -> Info(params = Map(
    ...
    "RLPF" -> Info(params = Map(
      "in"    -> Spec(dynamic = true),
      "freq"  -> Spec(lo = 10.0, hi = 20000.0),
      "rq"    -> Spec(lo = 0.01, hi = 100.0)    // arbitrary
    )),
    "RHPF" -> Info(params = Map(
      "in"    -> Spec(dynamic = true),
      "freq"  -> Spec(lo = 10.0, hi = 20000.0),
      "rq"    -> Spec(lo = 0.01, hi = 100.0)    // arbitrary
    )),
