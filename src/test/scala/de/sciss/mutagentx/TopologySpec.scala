...
//      assert(tc.edgeMap.get(vcd).fold(0)(_.size) === 1)
  ...
  ignore /* it */ should "really do that" in {
    ...
    val ti = ti16
    val si0 = ti.vertices.iterator.toList.mkString(",")
    val si1 = ti.edges   .iterator.toList.sortBy(_.toString  ).mkString(",")
    val si2 = ti.edgeMap .iterator.toList.sortBy(_.toString()).mkString(",")
